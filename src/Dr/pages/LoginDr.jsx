...
//             if (data.role === "doctor" || data.userType === "doctor") {
//                 navigate("/Dr")  // صفحة الدكتور
//             } else if (data.role === "manager" || data.userType === "manager") {
//                 navigate("/managment")  // صفحة الإدارة
//             } else {
//                 navigate("/")
//             }
//         } else {
...
export default function DrHome() {
    ...
    async function handleLogin(formData) {
        ...
        if (data.message === "success") {
            localStorage.setItem("token", data.token)
            setIsLoggedIn(true);
            navigate("/Dr")  // لو نجح الـ login = دكتور متسجل
        } else {
            // لو فشل = مش متسجل
            setErrMsg(data.message || "بيانات غير صحيحة")
            setTimeout(() => {
                navigate("/management")  // يحوله لصفحة management بعد ثانيتين
            }, 2000)
        }
    }
