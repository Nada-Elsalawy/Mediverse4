...
const DoctorDashboard = () => {
  ...
  const loadInitialData = async () => {
    ...
    setError(null);
    try {
      //  هجيب بيانات الطبيب من الشات بوت
      ...
      // هجيب بيانات امريض من الشات بوت
          ...
          try {
           ...
           console.log('🔍 Files response:', JSON.stringify(allFiles, null, 2));
            if (allFiles && allFiles.files && Array.isArray(allFiles.files)) {
              const fixedFiles = allFiles.files.map(f => ({...f, file_url: getFullUrl(f.file_url || f.url)}));
    ...
    try {
      ...
      if (response.patient) {
        ...
        try {
          ...
          const files = (allFiles?.files && Array.isArray(allFiles.files)) ? allFiles.files : (allFiles || []);
          const fixedFiles = files.map(f => ({...f, file_url: getFullUrl(f.file_url || f.url)}));
          setPatientMedicalFiles(fixedFiles);
          const xrays = fixedFiles.filter(f => f.file_type === 'xray');
          const labTests = fixedFiles.filter(f => f.file_type === 'lab_test');
          setXrayFiles(xrays);
            ...
            <div className="bg-[#e8f3f9] p-5 rounded-xl">
              ...
              <div className='flex justify-between pe-9'>
                ...
                <div>
                  ...
                  <p className='pb-4'>{doctorProfile?.name || 'Dr Nada'}</p>
                </div>
              </div>
              <div className='bg-white p-3'>
                <h2>Ai Analysis</h2>
                <div className="mt-2 space-y-3">
                  {(xrayFiles[currentXrayIndex]?.ai_analysis || 'No AI analysis available')
                    .split(/\d+\.\s+/)
                    .filter(Boolean)
                    .map((point, i) => {
                      const importantSplit = point.split(/⚠️\s*IMPORTANT:|IMPORTANT:/);
                      const mainText = importantSplit[0];
                      const important = importantSplit[1];
                      const titleMatch = mainText.match(/^\*\*(.+?)\*\*[:\s]*(.*)/s);
                      return (
                        <div key={i} className="text-sm text-slate-700">
                          <span className="font-bold text-slate-900">{i + 1}. {titleMatch ? titleMatch[1] : ''}</span>
                          {titleMatch ? (
                            <span className="text-slate-600"> {titleMatch[2].trim()}</span>
                          ) : (
                            <span> {mainText.trim()}</span>
                          )}
                          {important && (
                            <p className="mt-1 text-orange-600 font-semibold">⚠️ IMPORTANT: {important.trim()}</p>
                          )}
                        </div>
                      );
                    })
          ...
          <div className="bg-white rounded-3xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="bg-[#e8f3f9] p-5 rounded-xl">
              ...
              <div className='bg-white p-3'>
                ...
                <div className="space-y-3">
                  {(labTestFiles[currentLabIndex]?.ai_analysis || 'No AI analysis available')
                    .split(/\d+\.\s+/)
                    .filter(Boolean)
                    .map((point, i) => {
                      const importantSplit = point.split(/⚠️\s*IMPORTANT:|IMPORTANT:/);
