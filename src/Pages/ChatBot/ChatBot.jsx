...
// import { Input, Button } from '@heroui/react';
// import { sendConsultation, prepareConsultationData } from '../../APi/ChatBotApi';
// import { joinQueue, getPatientActiveQueue } from '../../QueueApi';
// import { getPatientId } from '../../getPatientId';
...
//   /**
//    * التعامل مع الحجز مع طبيب
//    */
//   const handleBookDoctor = async (doctor) => {
//     try {
//       console.log('🏥 Attempting to book with doctor:', doctor);
      
//       // التحقق من قائمة الانتظار النشطة للمريض
//       const activeQueue = await getPatientActiveQueue(patientId);
      
//       if (activeQueue) {
//         alert('لديك قائمة انتظار نشطة بالفعل. يرجى إنهاؤها أولاً.');
//         return;
//       }

//       // التحقق من حالة الطبيب
//       if (!doctor.current_patients || doctor.current_patients === 0) {
//         // الطبيب متاح فوراً
//         setNotificationData({
//           type: 'available',
//           doctor: doctor,
//           message: `الدكتور ${doctor.name} متاح الآن!`,
//           floor: doctor.floor,
//           room: doctor.room
//         });
//         setShowNotification(true);
        
//         // بعد 5 ثوانٍ، انقل للصفحة
//         setTimeout(() => {
//           setShowNotification(false);
//           // حفظ بيانات الطبيب في localStorage للاستخدام في صفحة الطبيب
...
//         console.log('✅ Joined queue:', queueResponse);
        
//         setNotificationData({
//           type: 'queue',
//           doctor: doctor,
//           queuePosition: queueResponse.position || doctor.current_patients + 1,
//           estimatedWait: doctor.estimated_wait_minutes,
//           queueId: queueResponse.queue_id
//         });
//         setShowNotification(true);
        
//         // بعد 3 ثوانٍ، انقل لصفحة الانتظار
//         setTimeout(() => {
//           setShowNotification(false);
...
//                     >
//                       {doctor.current_patients === 0 ? 'توجه للطبيب الآن' : `احجز مع Dr. ${doctor.name}`}
//                     </Button>
//                   </div>
//                 ))}
//               </div>
...
//   const patientId = getPatientId() || '124'; // ID تجريبي
//   const patientData = JSON.parse(localStorage.getItem("patient") || "{}");
//   const navigate = useNavigate();
...
//   /**
//    * التعامل مع الحجز مع طبيب - توجيه مباشر
//    */
//   const handleBookDoctor = async (doctor) => {
//     try {
...
//       // التحقق من حالة الطبيب
//       const isDoctorAvailable = !doctor.current_patients || doctor.current_patients === 0;
      
//       // حفظ بيانات الطبيب والمريض قبل التوجيه
...
//         // الطبيب متاح - توجيه مباشر
//         console.log('✅ Doctor available - redirecting to /Dr');
//         navigate('/Dr');
//       } else {
//         // الطبيب مشغول - إضافة للقائمة
//         console.log('⏱️ Doctor busy - joining queue');
...
//       if (error.message.includes('queue') || error.message.includes('قائمة')) {
//         alert('لديك قائمة انتظار نشطة. يرجى إنهاؤها من صفحة الانتظار أولاً.');
//       } else {
//         alert(`خطأ في الحجز: ${error.message}`);
//       }
//     }
...
//       const consultationData = prepareConsultationData(content, {
//         model: selectedModel,
//         patient_age: parseInt(patientData?.age) || 18,
//         patient_gender: patientData?.gender || "male",
//         patient_weight: parseFloat(patientData?.weight) || null,
//         patient_height: parseFloat(patientData?.height) || null,
//         chronic_diseases: ensureArray(patientData?.chronic_diseases),
//         allergies: ensureArray(patientData?.allergies),
//         current_medications: ensureArray(patientData?.medications || patientData?.current_medications),
//         use_rag: true,
//         top_k: 5,
//         ...(patientId && { patient_id: patientId })
//       });
...
//         ai_analysis: lastAIResponse ? {
//           diagnosis: lastAIResponse.diagnosis || '',
//           confidence: lastAIResponse.confidence_percentage || 0,
//           recommended_department: lastAIResponse.recommended_department || '',
//           severity: lastAIResponse.severity || 'moderate',
...
//       // 4. التوجيه بناءً على حالة الدكتور
//       if (isDoctorAvailable) {
...
//       } else {
//         // ⏱️ الطبيب مشغول - الانضمام للـ Queue
...
//       setMessages(prev => [...prev, { 
//         sender: "bot", 
//         text: `عذراً، حدث خطأ: ${error.message}. يرجى المحاولة مرة أخرى.`,
//         isError: true 
//       }]);
//     } finally {
//       setIsLoading(false);
...
//     if (!data || !data.assessment) {
//       return (
//         <div className="flex my-4 text-sm gap-3 justify-end">
//           <div className="p-4 bg-red-50 rounded-lg text-red-600 text-sm max-w-[70%]">
//             عذراً، لم نتمكن من معالجة الاستجابة بشكل صحيح
//           </div>
//           <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
//             <div className="rounded-full bg-gray-100 border p-1">⚠️</div>
//           </span>
//         </div>
//       );
//     }
...
//               <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
//                 {available_doctors.map((doctor, idx) => {
//                   // التحقق من حالة الدكتور من البيانات الموجودة
...
//       console.log('  - Has assessment?', !!response?.assessment);
//       console.log('  - Has preliminary_diagnosis?', !!response?.assessment?.preliminary_diagnosis);
//       console.log('  - Has severity?', !!response?.assessment?.severity);
//       console.log('  - Has specialty_required?', !!response?.assessment?.specialty_required);
      
//       // ✅ حفظ الـ AI Response
//       setLastAIResponse(response);
//       console.log('💾 Saved to lastAIResponse:', response);
